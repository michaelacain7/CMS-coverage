...
def scan_cms_newsroom() -> dict:
    ...
    result = {"success": False, "items_found": 0, "items": []}
    try:
        resp = CMSScanner.safe_get("https://www.cms.gov/newsroom/press-releases", accept_json=False, timeout=20)
        if resp is None:
...
def format_alert_embed(item: dict, analysis: dict) -> dict:
    ...
    if revenue_impact:
        fields.append({"name": "💵 Revenue Impact", "value": revenue_impact[:1024], "inline": False})

    url = item.get("url", "")

    # Build clean description — show source URL as clickable link, not raw JSON
    desc_parts = []
    if url:
        desc_parts.append(f"**📎 Source Document:** [View on CMS.gov]({url})")
