...
class HealthTracker:
    ...
    def should_alert_health(self) -> tuple[bool, str]:
        ...
        alerted_sources = self.status.get("_health_alerted_sources", {})
        for source, info in self.status.get("source_status", {}).items():
            consec = info.get("consecutive_failures", 0)
            if consec >= 3:
                # 2-hour cooldown: don't re-alert same source within 2 hours
                last_alerted = alerted_sources.get(source)
                if last_alerted:
                    try:
                        if (now - datetime.fromisoformat(last_alerted)).total_seconds() < 7200:
                            continue
                    except (ValueError, TypeError):
                        pass
                reasons.append(f"❌ Source `{source}` failing {consec}x in a row: {(info.get('error') or 'unknown')[:100]}")
            else:
                # Source recovered — clear cooldown so future failures alert fresh
                alerted_sources.pop(source, None)
        ...
        if reasons:
            for source, info in self.status.get("source_status", {}).items():
                if info.get("consecutive_failures", 0) >= 3:
                    alerted_sources[source] = now.isoformat()
            self.status["_health_alerted_sources"] = alerted_sources
            self.save()
...
def _extract_date_near_link(link_el) -> str:
    """Try to extract a date string from HTML elements near a link.

    Looks at: parent text, sibling elements, nearby date-classed elements.
    Returns the date string if found, empty string otherwise.
    """
    # Check parent and grandparent for date-like text
    for parent in [link_el.parent, getattr(link_el.parent, 'parent', None)]:
        if not parent:
            continue
        # Look for elements with 'date' in class name
        date_el = parent.find(class_=lambda c: c and "date" in str(c).lower())
        if date_el:
        ...
        # Look for <span> with date-like content near the link
        for span in parent.find_all(["span", "small", "em", "time"]):
            text = span.get_text(strip=True)
            if re.search(r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|'
                         r'January|February|March|April|June|July|August|September|'
                         r'October|November|December)\b.*\d{4}', text, re.IGNORECASE):
                return text
            ...
            if any(kw in text.lower() for kw in ["hcpcs file", "alpha-numeric", "update", "quarterly"]) or \
               any(kw in href.lower() for kw in [".zip", "hcpcs", "quarterly"]):
                ...
                result["items"].append({
                    ...
                    "description": f"Date: {_extract_date_near_link(link)}\nHCPCS Quarterly Update File: {href}" if _extract_date_near_link(link) else f"HCPCS Quarterly Update File: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in ["application summary", "coding", "determination", "drugs", "biologicals", "public meeting"]):
                ...
                result["items"].append({
                    "id": item_id, "title": text,
                    "description": f"Date: {_extract_date_near_link(link)}\nHCPCS Coding Decision/Application: {href}" if _extract_date_near_link(link) else f"HCPCS Coding Decision/Application: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in ["ncd", "coverage", "determination", "tracking", "decision memo", "proposed"]):
                ...
                result["items"].append({
                    "id": item_id, "title": text,
                    "description": f"Date: {_extract_date_near_link(link)}\nCMS Coverage Update: {href}" if _extract_date_near_link(link) else f"CMS Coverage Update: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    ...
                    "description": f"Date: {_extract_date_near_link(link)}\nDrug Negotiation Program: {href}" if _extract_date_near_link(link) else f"Drug Negotiation Program: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {_extract_date_near_link(link)}\nDrug Negotiation Guidance: {href}" if _extract_date_near_link(link) else f"Drug Negotiation Guidance: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                seen_hrefs.add(href)
                item_id = hashlib.md5(f"{text}:{href}".encode()).hexdigest()[:12]
                date_text = _extract_date_near_link(link)
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {date_text}\nPart B ASP Drug Pricing File: {href}" if date_text else f"Part B ASP Drug Pricing File: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {date_text}\nMA Rate/Payment Update: {href}" if date_text else f"MA Rate/Payment Update: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {date_text}\nMA Star Ratings: {href}" if date_text else f"MA Star Ratings: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if any(kw in href.lower() for kw in [
                    ...
                    result["items"].append({
                        "id": item_id, "title": text[:300],
                        "description": f"Date: {_extract_date_near_link(link)}\nCMMI: {href}" if _extract_date_near_link(link) else f"CMMI: {href}",
                        "source": source, "url": href,
                    })
        except Exception as e:
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if any(kw in text.lower() for kw in [
                    ...
                    result["items"].append({
                        "id": item_id, "title": text[:300],
                        "description": f"Date: {_extract_date_near_link(link)}\nHospital Payment Update: {href}" if _extract_date_near_link(link) else f"Hospital Payment Update: {href}",
                        "source": source, "url": href,
                    })
        except Exception as e:
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {_extract_date_near_link(link)}\nCMS Transmittal: {href}" if _extract_date_near_link(link) else f"CMS Transmittal: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(kw in text.lower() for kw in [
                ...
                result["items"].append({
                    "id": item_id, "title": text[:300],
                    "description": f"Date: {_extract_date_near_link(link)}\nDMEPOS Update: {href}" if _extract_date_near_link(link) else f"DMEPOS Update: {href}",
                    "source": source, "url": href,
                })
        result["items_found"] = len(result["items"])
        result["success"] = True
...
def format_alert_embed(item: dict, analysis: dict) -> dict:
    ...
    # Validate URL — Discord rejects embeds with malformed URLs (spaces, missing scheme)
    if url and (" " in url or not url.startswith("http")):
        log.warning(f"Skipping malformed embed URL: {url[:100]}")
        url = ""
    ...
    # Ensure no field has an empty value (Discord rejects these)
    for f in fields:
        if not f.get("value") or not f["value"].strip():
            f["value"] = "N/A"
...
def enrich_item_content(item: dict) -> str:
    ...
    try:
        # Skip binary files (but NOT PDFs — we extract text from those)
        if any(url.lower().endswith(ext) for ext in [".zip", ".xlsx", ".xls", ".csv"]):
            return base_desc

        # Use longer timeout for PDFs
        fetch_timeout = 15 if url.lower().endswith(".pdf") else 8
        resp = CMSScanner.safe_get(url, accept_json=False, timeout=fetch_timeout)
        if resp is None:
            return base_desc

        content_type = resp.headers.get("Content-Type", "")

        # For PDFs, extract text content for AI analysis
        if "pdf" in content_type or url.lower().endswith(".pdf"):
            try:
                ...
                log.debug(f"PDF text extraction failed for {url}: {e}")
...
_DATE_FIELDS = [
    ...
    "created", "createdDate", "created_date",
    "datePosted", "dateModified", "datePublished",
    "revision_date", "revisionDate", "start_date", "startDate",
    "notice_date", "noticeDate", "issue_date", "issueDate",
]

# Regex patterns that match date-like strings in free text
_DATE_REGEXES = [
    ...
    re.compile(r'\b((?:January|February|March|April|May|June|July|August|September|October|November|December|'
               r'Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\.?\s+\d{1,2},?\s+20\d{2})\b', re.IGNORECASE),
    # 4 March 2026
    re.compile(r'\b(\d{1,2}\s+(?:January|February|March|April|May|June|July|August|September|October|November|December|'
               r'Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\.?\s+20\d{2})\b', re.IGNORECASE),
]

# URL patterns that contain dates
_URL_DATE_REGEXES = [
    re.compile(r'/(\d{4})/(\d{2})/(\d{2})/'),      # /2026/03/04/
    re.compile(r'/(\d{4})/(\d{2})/'),                # /2026/03/
    re.compile(r'[?&]date=(\d{4}-\d{2}-\d{2})'),    # ?date=2026-03-04
    re.compile(r'/(\d{4}-\d{2}-\d{2})'),             # /2026-03-04 in path
]

# Sources that ONLY show current/recent content by design — they don't have archives.
# If a NEW item appears on these pages, it is inherently recent.
# These get "today" as their date if no explicit date is found.
# NOTE: Archive pages (Star Ratings, ASP Pricing, HCPCS, MA Rates, etc.) are NOT
# live feeds — they list years of historical documents. Those must find a real date.
LIVE_FEED_SOURCES = {
    "CMS Newsroom",         # Only shows recent press releases
    "Federal Register",     # API returns recent publications only
    "CMS Enforcement Actions",  # Only shows active enforcement actions
}


def _try_parse_date(raw: str) -> datetime | None:
    """Try to parse a single date string using all known formats."""
    if not raw or not isinstance(raw, str):
        return None
    raw = raw.strip()

    # Handle pure-digit sort-format dates like "20200417101916" or "20241127"
    if raw.isdigit():
        if len(raw) >= 14:
            try:
                return datetime.strptime(raw[:14], "%Y%m%d%H%M%S").replace(tzinfo=EST)
            except ValueError:
                pass
        if len(raw) >= 8:
            try:
                return datetime.strptime(raw[:8], "%Y%m%d").replace(tzinfo=EST)
            except ValueError:
                pass
        return None
    ...
    for fmt in _DATE_FORMATS:
        try:
            return datetime.strptime(clean, fmt).replace(tzinfo=EST)
        except ValueError:
            continue
    return None
...
def extract_item_date(item: dict) -> datetime | None:
    """Try hard to extract the document's actual date from all available data.

    Checks in order:
    1. Structured JSON date fields in item description
    2. Top-level item fields
    3. ALL values in the JSON description (recursive scan for date-like values)
    4. Regex scan of description text for date patterns
    5. Regex scan of title for date patterns
    6. Date patterns in the URL
    7. For live-feed sources: returns today's date as fallback
    ...
    # ── 1. Structured JSON date fields from description ──
    try:
        if desc.lstrip().startswith("{") or desc.lstrip().startswith("["):
            parsed_json = json.loads(desc)
    ...
    # Try the priority candidates first (structured date fields are most reliable)
    for raw in candidates:
        dt = _try_parse_date(raw)
        if dt:
            return dt

    # ── 3. Deep scan ALL values in the JSON description for date-like strings ──
    if parsed_json and isinstance(parsed_json, dict):
        for key, val in parsed_json.items():
            if key in _DATE_FIELDS:
                continue  # already tried
            if isinstance(val, str) and len(val) >= 8 and len(val) <= 30:
    ...
    for regex in _DATE_REGEXES:
        ...
        if match:
            dt = _try_parse_date(match.group(1))
            if dt:
                return dt

    # ── 5. Regex scan of title ──
    title = item.get("title", "")
    for regex in _DATE_REGEXES:
    ...
    # ── 6. Date patterns in URL ──
    url = item.get("url", "")
    if url:
        for regex in _URL_DATE_REGEXES:
            match = regex.search(url)
            if match:
                groups = match.groups()
                try:
                    if len(groups) == 3:
                        dt = datetime(int(groups[0]), int(groups[1]), int(groups[2]), tzinfo=EST)
                        return dt
                    elif len(groups) == 2:
                        dt = datetime(int(groups[0]), int(groups[1]), 1, tzinfo=EST)
                        return dt
                    elif len(groups) == 1:
                        dt = _try_parse_date(groups[0])
                        if dt:
                            return dt
                except (ValueError, TypeError):
                    pass

    # ── 7. Year from URL filename (e.g. /2025-star-ratings-data-tables.zip) ──
    if url:
        # Match year at start of filename segment: /2025-something or /files/zip/2025-
        year_in_url = re.search(r'/(\d{4})-[a-zA-Z]', url)
        if not year_in_url:
            # Also try: ?year=2025, /cy2025, /fy2025, /pfs2026
            year_in_url = re.search(r'(?:[?&]year=|/[cfp][ys]?)(\d{4})', url, re.IGNORECASE)
        if year_in_url:
            try:
                year = int(year_in_url.group(1))
                if 2015 <= year <= 2035:
                    # Use Jan 1 of that year — CMS "year X" docs are released in fall of X-1,
                    # so by the time year X starts, they're already months old
                    return datetime(year, 1, 1, tzinfo=EST)
            except (ValueError, TypeError):
                pass

    # ── 8. Year or quarter from title (e.g. "2025 Star Ratings", "Q3 2026") ──
    title = item.get("title", "")
    combined_text = f"{title} {desc}"

    # Quarter detection: "Q1 2026", "Q3 FY2025", "1st Quarter 2025"
    quarter_match = re.search(r'\b[Qq]([1-4])\s*(?:FY|CY)?\s*(\d{4})\b', combined_text)
    if not quarter_match:
        quarter_match = re.search(r'\b([1-4])(?:st|nd|rd|th)\s+[Qq]uarter\s+(\d{4})\b', combined_text)
    if quarter_match:
        try:
            q = int(quarter_match.group(1))
            year = int(quarter_match.group(2))
            if 2015 <= year <= 2035:
                month = {1: 2, 2: 5, 3: 8, 4: 11}[q]  # middle of quarter
                return datetime(year, month, 15, tzinfo=EST)
        except (ValueError, KeyError):
            pass

    # Bare year in title: "2025 Star Ratings Data Tables", "CY 2026 Final Rule"
    # Must be preceded by start-of-string, space, or CY/FY prefix
    year_in_title = re.search(r'(?:^|\s|CY|FY)\s*(\d{4})\b', title)
    if year_in_title:
        try:
    ...
    # ── 9. Live-feed source fallback: if this is a live-feed page, use today ──
    source = item.get("source", "")
    if source in LIVE_FEED_SOURCES:
...
def is_item_recent(item: dict, max_age_days: int = MAX_ITEM_AGE_DAYS) -> bool:
    ...
    Returns True only if a date was found AND it's within max_age_days.
    Items with no extractable date are NOT allowed through.
    """
    item_date = extract_item_date(item)
    if item_date is None:
        return False  # No date found — do not alert on undated items
...
def run_scan_cycle(seen: dict, baseline_only: bool = False) -> dict:
    """Run a complete scan cycle across all sources.
    
    If baseline_only=True, items are learned (added to seen) but NO AI analysis
    or Discord alerts are sent. Used on first run to silently establish baseline.
    """
    log.info("Starting scan cycle..." + (" (baseline — no alerts)" if baseline_only else ""))
    source_results = {}
    new_items = []
    any_success = False
    ...
    # Baseline mode: skip all AI analysis and alerting, just count
    if baseline_only:
        for item in new_items:
            if not is_item_recent(item):
                stale_skipped += 1
        log.info(f"Baseline scan complete: {len(new_items)} items learned, {stale_skipped} stale, 0 alerts (suppressed)")
    ...
    for item in new_items:
        ...
        if not is_item_recent(item):
            ...
            if item_date:
                ...
                log.debug(f"Skipping stale item (dated {date_str}): {item.get('title', '')[:80]}")
            else:
                log.debug(f"Skipping undated item (no date found): {item.get('source', '')} | {item.get('title', '')[:80]}")
            stale_skipped += 1
            continue
...
def main():
    ...
    if not seen:
        ...
        run_scan_cycle(seen, baseline_only=True)
