...
def enrich_item_content(item: dict) -> str:
    """Fetch the actual page/document content for a new item to give AI real details.
    Returns enriched description with actual content extracted."""
    url = item.get("url", "")
    ...
    try:
        ...
        # For PDFs, just note it's a PDF - can't easily extract
...
1. What SPECIFICALLY changed? (exact HCPCS/CPT codes, drug names, procedure names, payment rates, coverage decisions)
...
- ENFORCEMENT/SANCTION items (enrollment freezes, CMPs, terminations) are almost ALWAYS HIGH impact if they name a public company
- For sanctions: identify the parent public company ticker even if the sanction names a subsidiary (e.g. Elevance Health = ELV, UnitedHealth = UNH, Humana = HUM, CVS/Aetna = CVS, Centene = CNC, Molina = MOH, Cigna = CI)
...
{{
    ...
    "summary": "SPECIFIC summary: what code/drug/rate changed, how it affects the named company's revenue. Never be generic.",
    "category": "NEW_CODE/COVERAGE_CHANGE/PRODUCT_APPROVAL/PRICING/POLICY/MEETING/DRUG_NEGOTIATION/RATE_CHANGE/MODEL_CHANGE/PAYMENT_UPDATE/ENFORCEMENT/SANCTION/OTHER"
}}"""
...
def scan_drug_price_negotiation() -> dict:
    """Scan Medicare Drug Price Negotiation Program page for selected drugs, negotiated prices, new cycles."""
...
def scan_drug_negotiation_guidance() -> dict:
    """Scan Drug Price Negotiation guidance page for new guidance docs, timelines, selections."""
...
def scan_asp_pricing_files() -> dict:
    """Scan Part B ASP drug pricing files page for new quarterly pricing files."""
...
def scan_ma_rate_announcements() -> dict:
    """Scan Medicare Advantage rate stats page for Advance Notices and Rate Announcements."""
...
def scan_star_ratings() -> dict:
    """Scan CMS Star Ratings page for Medicare Advantage quality ratings updates."""
...
def scan_cmmi_models() -> dict:
    """Scan CMS Innovation Center pages for new/modified payment models and announcements."""
...
def scan_ipps_opps() -> dict:
    """Scan CMS hospital payment (IPPS/OPPS) pages for new rules, NTAPs, payment updates."""
...
def scan_cms_transmittals() -> dict:
    """Scan CMS transmittals/change requests for billing and payment implementation changes."""
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if not text or len(text) < 10:
                continue
            # Transmittal links typically contain 'r' + numbers + 'cp' or similar
            if any(kw in text.lower() for kw in [
...
def scan_dmepos() -> dict:
    """Scan DMEPOS competitive bidding page for new bid results and fee schedule updates."""
...
# ─── CMS Enforcement & Sanctions Scanners ────────────────────────────────────

def scan_enforcement_actions() -> dict:
    """Scan CMS Part C/D Enforcement Actions page for sanctions, CMPs, and terminations.
    This is the page where CMS posts intermediate sanctions (like the Elevance Health enrollment
    freeze), civil money penalties, and contract terminations against MA/Part D plan sponsors.
    These are among the most stock-moving CMS publications for managed care companies."""
    ...
    soup = None
    for url in enforcement_urls:
        try:
            resp = CMSScanner.safe_get(url, accept_json=False, timeout=12)
            if resp and resp.status_code == 200:
                soup = BeautifulSoup(resp.text, "html.parser")
                break
        except Exception:
    ...
    if soup is None:
        result["error"] = "All enforcement URLs returned errors"
        return result
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            # Match enforcement-related documents: sanction PDFs, CMP notices, termination letters
            is_enforcement = False

            # PDF documents (sanctions, CMP notices are typically posted as PDFs)
            if href.lower().endswith(".pdf"):
                lower_text = text.lower()
                lower_href = href.lower()
                if any(kw in lower_text or kw in lower_href for kw in [
                    "sanction", "cmp", "civil money", "penalty", "termination",
                    "enforcement", "intermediate", "suspension", "corrective",
                    "notice of intent", "noncompliance", "non-compliance",
                ]):
                    is_enforcement = True
                # Also catch company-named sanction PDFs like "elevancehealthsanction02272026.pdf"
                if "sanction" in lower_href:
                    is_enforcement = True

            # Links to enforcement sub-pages
            if any(kw in href.lower() for kw in [
                "enforcement", "sanction", "civil-money", "cmp",
                "termination-notice", "intermediate-sanction",
            ]):
                is_enforcement = True
            ...
            result["items"].append({
                "id": item_id,
                "title": f"⚠️ ENFORCEMENT: {text[:280]}",
                "description": f"CMS Enforcement Action Document\nURL: {href}",
                "source": source,
                "url": href,
            })

        # Also scan the page text for any embedded document references
        # CMS sometimes lists enforcement actions in tables or plain text
        main_content = soup.find("main") or soup.find(attrs={"role": "main"}) or soup.find(id="main-content")
        if main_content:
            for link in main_content.find_all("a", href=True):
                ...
                if href in seen_hrefs:
                    continue
                # Catch ANY PDF in the main content area of the enforcement page
                if href.lower().endswith(".pdf"):
                    seen_hrefs.add(href)
                    item_id = hashlib.md5(href.encode()).hexdigest()[:12]
                    result["items"].append({
                        "id": item_id,
                        "title": f"⚠️ ENFORCEMENT DOC: {text[:280]}",
                        "description": f"Document on CMS Enforcement Actions page\nURL: {href}",
                        "source": source,
                        "url": href,
                    })
        ...
        result["error"] = str(e)
        log.error(f"CMS Enforcement Actions scan error: {e}")
    return result


def scan_compliance_actions() -> dict:
    """Scan CMS Part C/D Compliance Actions page for warning letters and CAPs.
    Warning letters and corrective action plans often precede sanctions and can
    signal upcoming enforcement actions against managed care companies."""
    source = "CMS Compliance Actions"
    result = {"success": False, "items_found": 0, "items": []}
    ...
    if soup is None:
        result["error"] = "All compliance action URLs returned errors"
        return result
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            is_compliance = False
            # Warning letters, CAPs, compliance reports
            if any(kw in lower_text or kw in lower_href for kw in [
                "warning letter", "corrective action", "cap report",
                "compliance action", "non-compliance", "noncompliance",
                "ad hoc", "monthly report", "warning",
            ]):
                is_compliance = True

            # PDF/xlsx documents on this page
            if href.lower().endswith((".pdf", ".xlsx", ".xls", ".csv")):
                if any(kw in lower_text or kw in lower_href for kw in [
                    "warning", "cap", "compliance", "corrective", "letter",
                ]):
                    is_compliance = True
            ...
            result["items"].append({
                "id": item_id,
                "title": f"📋 COMPLIANCE: {text[:280]}",
                "description": f"CMS Compliance Action Document\nURL: {href}",
                "source": source,
                "url": href,
            })
        ...
        result["error"] = str(e)
        log.error(f"CMS Compliance Actions scan error: {e}")
    return result


def scan_cms_enforcement_documents() -> dict:
    """Scan CMS /files/document/ path for newly posted sanction and enforcement PDFs.
    CMS posts sanction letters as PDFs under /files/document/ with naming patterns like
    'elevancehealthsanction02272026.pdf'. This scanner checks the CMS audits/compliance
    parent page and the program audits page for links to these documents."""
    source = "CMS Enforcement Documents"
    result = {"success": False, "items_found": 0, "items": []}

    # Pages that link to enforcement/audit documents
    doc_pages = [
        "https://www.cms.gov/medicare/audits-compliance/part-c-d",
    ...
    for page_url in doc_pages:
        try:
            resp = CMSScanner.safe_get(page_url, accept_json=False, timeout=12)
            if resp is None:
                continue
            any_page_ok = True
            ...
            for link in soup.find_all("a", href=True):
                ...
                # Match PDF documents with enforcement keywords in filename or link text
                if lower_href.endswith(".pdf"):
                    filename = lower_href.split("/")[-1]
                    if any(kw in filename for kw in [
                        "sanction", "cmp", "enforcement", "penalty",
                        "termination", "audit-report", "audit-enforcement",
                        "program-audit",
                    ]) or any(kw in lower_text for kw in [
                        "sanction", "civil money", "penalty", "enforcement",
                        "termination", "audit report", "cmp",
                    ]):
                        seen_hrefs.add(href)
                        item_id = hashlib.md5(href.encode()).hexdigest()[:12]
                        all_items.append({
                            "id": item_id,
                            "title": f"🚨 ENFORCEMENT DOC: {text[:280]}",
                            "description": f"CMS Enforcement/Audit Document: {filename}\nURL: {href}",
                            "source": source,
                            "url": href,
                        })

        except Exception as e:
            log.debug(f"Enforcement doc scan error for {page_url}: {e}")
            continue

    result["items"] = all_items
    result["items_found"] = len(all_items)
    result["success"] = any_page_ok
    if not any_page_ok:
        result["error"] = "All enforcement document pages returned errors"
    return result
...
ALL_SCANNERS = {
    ...
    # Tier 1: Enforcement & Sanctions (massive movers for managed care stocks)
    "CMS Enforcement Actions": scan_enforcement_actions,
    "CMS Compliance Actions": scan_compliance_actions,
    "CMS Enforcement Documents": scan_cms_enforcement_documents,
}


def run_scan_cycle(seen: dict, baseline: bool = False) -> dict:
    """Run a complete scan cycle across all sources.
    If baseline=True, just learn existing items without AI analysis or alerts.
    """
    log.info(f"Starting scan cycle...{' (BASELINE - learning existing items)' if baseline else ''}")
    source_results = {}
    new_items = []
    any_success = False
    ...
    # Process new items through AI analysis (skip on baseline)
    alerts_sent = 0
    if baseline:
        log.info(f"Baseline scan: learned {len(new_items)} items, skipping AI analysis")
    else:
        for item in new_items:
            ...
            if analysis is None:
                ...
                if item["source"] in (
                    ...
                    "MA Rate Announcements", "MA Star Ratings",
                    "CMS Enforcement Actions", "CMS Compliance Actions", "CMS Enforcement Documents",
                ):
                    analysis = {
...
def main():
    ...
    send_discord_alert(None, HEALTH_WEBHOOK, embed={
        ...
        "description": (
            ...
            "• **CMS Enforcement Actions & Sanctions** 🚨\n"
            "• **CMS Compliance Actions (Warning Letters/CAPs)** 📋"
        ),
        "color": 0x00CC66,
        "timestamp": datetime.now(EST).isoformat(),
    })
    ...
    if not seen:
        ...
        run_scan_cycle(seen, baseline=True)
