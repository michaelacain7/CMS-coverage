...
# Sources that only show current/recent content by design — if a NEW item appears
# on these pages, it is inherently recent (the page itself is a live feed).
# These get "today" as their date if no explicit date is found.
LIVE_FEED_SOURCES = {
    "CMS Newsroom", "Federal Register",
    "HCPCS Updates", "HCPCS Coding", "CMS Coverage Page",
    "Drug Price Negotiation", "Drug Negotiation Guidance",
    "ASP Drug Pricing", "MA Rate Announcements", "MA Star Ratings",
    "CMS Enforcement Actions", "CMMI Innovation Models",
    "IPPS/OPPS Hospital Payments", "CMS Transmittals", "DMEPOS Bidding",
}
...
def extract_item_date(item: dict) -> datetime | None:
    ...
    # ── 7. Live-feed source fallback: if this is a live-feed page, use today ──
    source = item.get("source", "")
