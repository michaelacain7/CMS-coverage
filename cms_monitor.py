...
def scan_lcd_whats_new() -> dict:
    ...
    # The whats-new endpoint can require date params — try multiple endpoints
    lcd_endpoints = [
    ...
    for path in lcd_endpoints:
        try:
            resp = CMSScanner.safe_get(f"{CMS_API_BASE}{path}")
            if resp is None:
                continue
    ...
    result["items_found"] = len(result["items"])
    result["success"] = any_worked or len(result["items"]) > 0
