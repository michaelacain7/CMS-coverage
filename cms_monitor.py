...
{{
    ...
    "category": "NEW_CODE/COVERAGE_CHANGE/PRODUCT_APPROVAL/PRICING/POLICY/MEETING/DRUG_NEGOTIATION/RATE_CHANGE/MODEL_CHANGE/PAYMENT_UPDATE/OTHER"
}}"""
...
def scan_star_ratings() -> dict:
    ...
    try:
        resp = CMSScanner.safe_get(
            "https://www.cms.gov/medicare/health-plans/medicareadvtgspecratestats/quality/stars",
            accept_json=False, timeout=10,
        )
        if resp is None:
            resp = CMSScanner.safe_get(
                "https://www.cms.gov/medicare/health-plans/part-c-d-performance-data",
                accept_json=False, timeout=10,
            )
        if resp is None:
            result["error"] = "Page request failed"
        ...
        for link in soup.find_all("a", href=True):
            ...
            if not text or len(text) < 10:
                continue
            if any(kw in text.lower() for kw in [
                "star rating", "quality", "part c", "part d",
                "performance", "display measure", "technical notes",
                "fact sheet", "cut point",
...
def scan_cmmi_models() -> dict:
    """Scan CMS Innovation Center models page for new/modified payment models."""
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if not text or len(text) < 5:
                continue
            # Capture model links
            if "/innovation/innovation-models/" in href or "/priorities/innovation/models/" in href:
                if not href.startswith("http"):
                    href = f"https://www.cms.gov{href}"
                if href in seen_hrefs:
                    continue
        ...
        result["error"] = str(e)
        log.error(f"CMMI models scan error: {e}")
    return result
...
def scan_dmepos() -> dict:
    ...
    try:
        resp = CMSScanner.safe_get(
            "https://www.cms.gov/medicare/payment/durable-medical-equipment-prosthetics-orthotics-supplies-dmepos/dmepos-competitive-bidding",
            accept_json=False, timeout=10,
        )
        if resp is None:
            resp = CMSScanner.safe_get(
