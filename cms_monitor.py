...
def scan_lcd_whats_new() -> dict:
    """Scan the LCD What's New report for recent Local Coverage changes."""
    source = "LCD What's New"
    result = {"success": False, "items_found": 0, "items": []}
    try:
        resp = CMSScanner.safe_get(f"{CMS_API_BASE}/v1/reports/local-coverage-whats-new/")
        if resp is None:
            result["error"] = "API request failed"
            return result
        ...
        result["error"] = str(e)
        log.error(f"LCD What's New scan error: {e}")
    return result
...
def scan_cms_newsroom() -> dict:
    ...
    try:
        ...
        # Look for press release links/items
...
def scan_federal_register_cms() -> dict:
    ...
    result = {"success": False, "items_found": 0, "items": []}
    try:
        # Federal Register has a public API
